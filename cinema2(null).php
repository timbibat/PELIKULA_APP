...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($firstName) || empty($lastName) || empty($showtime) || empty($seat) || empty($email) || empty($quantity)) {
        ...
        if (!$user) {
            ...
            $user_id = $user['id'];
            $showdate = date('Y-m-d'); // Always use today's date for showdate
            // Insert booking - now includes showdate in the insert statement
...
<div class="container d-flex justify-content-center mt-3">
  <div class="card mb-3" style="width: 100cap;">
    <div class="ratio ratio-16x9">
      <video
        src="videos/The Conjuring_ Last Rites _ Official Trailer.mp4"
        title="Movie Trailer"
        autoplay
        class="locked-video"
        tabindex="-1"
    ...
    <div class="card-body">
      <h6>Paranormal investigators Ed and Lorraine Warren take on one last terrifying case involving mysterious entities they must confront.</h6>
      <p class="card-text"><?= htmlspecialchars($movie['description'] ?? '') ?></p>
      <p class="card-text"><small class="text-body-secondary">Run Time: <?= htmlspecialchars($movie['duration'] ?? '2 hours 17 minutes') ?></small></p>
    </div>
  </div>
</div>
<div class="container d-flex justify-content-center">
  <div class="card d-flex" style="width: 100cap;">
    <div class="card-body">
      <a href="advanced_booking.php?id=2" class="btn btn-success mb-3">Book In Advance</a>
